...
public class EntryResourceIT {
	 ...
	 * @Test
	 * 
	 * @Transactional public void searchEntry() throws Exception { // Initialize the
	 * database entryRepository.saveAndFlush(entry);
	 * when(mockEntrySearchRepository.search(queryStringQuery("id:" +
	 * entry.getId()), PageRequest.of(0, 20))) .thenReturn(new
	 * PageImpl<>(Collections.singletonList(entry), PageRequest.of(0, 1), 1)); //
	 * Search the entry
	 * restEntryMockMvc.perform(get("/api/_search/entries?query=id:" +
	 * entry.getId())) .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 * .andExpect(jsonPath("$.[*].id").value(hasItem(entry.getId().intValue())))
