...
private final EntryRepository entryRepository;
	 ...
	 * 
	 * public EntryResource(EntryRepository entryRepository, EntrySearchRepository
	 * entrySearchRepository) { this.entryRepository = entryRepository;
...
*/
	/*
	  @GetMapping("/_search/entries") public ResponseEntity<List<Entry>>
	  searchEntries(@RequestParam String query, Pageable pageable, @RequestParam
	  MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
	  log.debug("REST request to search for a page of Entries for query {}",
	  query); Page<Entry> page =
	  entrySearchRepository.search(queryStringQuery(query), pageable); HttpHeaders
	  headers =
	  PaginationUtil.generatePaginationHttpHeaders(uriBuilder.queryParams(
	  queryParams), page); return
